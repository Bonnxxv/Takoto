...
function ExportAudio(outputDir, inputTracks)
    ...
    project:SetRenderSettings({
        TargetDir = outputDir,
        CustomName = "takoto-exported-audio",
...
function LaunchApp()
    if ffi.os == "Windows" then
        ...
        if result_open > 32 then
            print("Takoto launched successfully.")
        else
            print("Failed to launch Takoto. Error code:", result_open)
            return
        end
    else
        -- MacOS & Linux
...
function StartServer()
    ...
    print("Takoto server is listening on port: ", PORT)
    ...
    print("Shutting down Takoto Link server...")
    server:close()
...
local Takoto = {
    Init = function(self, executable_path, resources_folder, dev_mode)
        ...
        assets_path = join_path(resources_path, "Takoto")
        StartServer()
    end
}
