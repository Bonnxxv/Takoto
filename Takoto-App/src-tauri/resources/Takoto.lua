...
if os_name == "Windows" then
    ...
    -- Get path to the main Takoto app and modules
    local storage_path = os.getenv("APPDATA") ..
        "\\Blackmagic Design\\DaVinci Resolve\\Support\\Fusion\\Scripts\\Utility\\Takoto"
    local install_path = assert(read_file(join_path(storage_path, "install_path.txt")))
    app_executable = install_path .. "\\Takoto.exe"
    resources_folder = install_path .. "\\resources"
elseif os_name == "OSX" then
    app_executable = "/Applications/Takoto.app"
    resources_folder = app_executable .. "/Contents/Resources/resources"
else
    app_executable = "/usr/bin/takoto"
    resources_folder = "/usr/lib/takoto/resources"
end

-- temporarily redefine path for dev_mode
if DEV_MODE then
    resources_folder = os.getenv("HOME") .. "/Documents/TakotoV3/Takoto-App/src-tauri/resources"
end
...
-- Launch Takoto
local Takoto = require("takoto_core")
Takoto:Init(app_executable, resources_folder, DEV_MODE)
